...
async def monitor_server_condition(ctx):
    ...
    for player_id, player in players:
        ...
        if players is not None and fps < target_fps:
            if (
                player_vehicles is not None
                and len(player_vehicles) > max_vehicles_per_player
            ):
                await announce(
...
async def monitor_rp_mode(ctx):
    ...
    for player_id, player in players:
        ...
        player_vehicles = await list_player_vehicles(
            ctx["http_client_mod"], player_id, active=True
        )
        ...
        is_autopilot = any(
            [
                v.get("isLastVehicle")
                and v.get("bIsAIDriving")
                and not is_position_zero(v.get("position"))
