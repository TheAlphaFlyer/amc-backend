...
async def monitor_locations(ctx):
  ...
  guid_to_player_info = {p['CharacterGuid']: p for p in players if p.get('CharacterGuid')}
  ...
  characters = {
    c.guid: c
    async for c in Character.objects.select_related('player')
      .filter(guid__in=guid_to_player_info.keys())
  }
  ...
  for guid, player_info in guid_to_player_info.items():
    ...
    if character.last_location:
      await _check_pois_and_portals(character, character.last_location, new_point, ctx)

    # Queue for bulk operations
    new_locations.append(CharacterLocation(
      character=character,
      location=new_point,
  ...
  if new_locations:
    await CharacterLocation.objects.abulk_create(new_locations, ignore_conflicts=True)
